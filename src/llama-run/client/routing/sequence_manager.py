...
from llama-run.client.config import ClientConfig
from llama-run.client.routing.sequence_info import RemoteSequenceInfo
from llama-run.client.routing.spending_policy import NoSpendingPolicy
from llama-run.data_structures import ModuleUID, RemoteSpanInfo, ServerState
from llama-run.server.handler import TransformerConnectionHandler
from llama-run.utils.dht import get_remote_module_infos
...
class SequenceManagerConfig(ClientConfig):
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "llama-run.client.routing.SequenceManagerConfig has been moved to llama-run.ClientConfig. "
...
class RemoteSequenceManager:
    ...
    def get_request_metadata(
        ...
        :param args_structure: the structure of flattened tensors from pack_args_kwargs in llama-run.utils.packaging
...
class MissingBlocksError(RuntimeError):
    def __init__(self, block_indices: Union[int, Sequence[int]]):
        super().__init__(
            ...
            f"You can check the public swarm's state at https://health.llama-run.dev "
            f"If there are not enough servers, please connect your GPU: "
            f"https://github.com/bigscience-workshop/llama-run#connect-your-gpu-and-increase-llama-run-capacity "
        )
