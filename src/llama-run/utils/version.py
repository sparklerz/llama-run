...
def validate_version() -> None:
    logger.info(f"Running {TextStyle.BOLD}Petals {llama-run.__version__}{TextStyle.RESET}")
    try:
        r = requests.get("https://pypi.python.org/pypi/llama-run/json")
        r.raise_for_status()
        response = r.json()
        ...
        if parse(llama-run.__version__) < latest:
            logger.info(
                f"A newer version {latest} is available. Please upgrade with: "
                f"{TextStyle.BOLD}pip install --upgrade llama-run{TextStyle.RESET}"
            )
...
def get_compatible_model_repo(model_name_or_path: Union[str, os.PathLike, None]) -> Union[str, os.PathLike, None]:
    ...
    match = re.fullmatch(r"(bigscience/.+)-llama-run", str(model_name_or_path))
    if match is None:
