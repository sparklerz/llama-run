...
from llama-run.data_structures import CHAIN_DELIMITER, UID_DELIMITER, ModelInfo, ServerInfo, ServerState, parse_uid
from llama-run.server import block_selection
from llama-run.server.backend import TransformerBackend, merge_inference_pools_inplace
from llama-run.server.block_utils import get_block_size, resolve_block_dtype
from llama-run.server.from_pretrained import load_pretrained_block
from llama-run.server.handler import TransformerConnectionHandler
from llama-run.server.memory_cache import MemoryCache
from llama-run.server.reachability import ReachabilityProtocol, check_direct_reachability, validate_reachability
from llama-run.server.throughput import get_dtype_name, get_server_throughput
from llama-run.utils.auto_config import AutoDistributedConfig
from llama-run.utils.convert_block import QuantType, check_device_balance, convert_block
from llama-run.utils.dht import declare_active_modules, get_remote_module_infos
from llama-run.utils.misc import get_size_in_bytes
from llama-run.utils.ping import PingAggregator
from llama-run.utils.random import sample_up_to
from llama-run.utils.version import get_compatible_model_repo
...
class Server:
    ...
    def _choose_num_blocks(self) -> int:
        ...
        if self.adapters:
            # Delay import of llama-run.utils.peft to avoid unnecessary import of bitsandbytes
            from llama-run.utils.peft import estimate_adapter_memory_per_block
