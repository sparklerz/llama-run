...
from llama-run.constants import REACHABILITY_API_URL
...
def validate_reachability(peer_id, wait_time: float = 7 * 60, retry_delay: float = 15) -> None:
    ...
    for attempt_no in range(math.floor(wait_time / retry_delay) + 1):
        try:
            ...
            if response["success"]:
                logger.info("Server is reachable from the Internet. It will appear at https://health.llama-run.dev soon")
                return
            ...
            logger.warning(f"Skipping reachability check because health.llama-run.dev is down: {repr(e)}")
            return
    ...
    raise RuntimeError(
        ...
        f"        python -m llama-run.cli.run_server ... --public_ip {response['your_ip']} --port 31337\n"
